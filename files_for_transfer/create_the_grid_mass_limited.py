...
print('    Make sure this list has been updated to include recent models!!')
data_dir = '/data/beegfs/astro-storage/groups/rix/dimoff/BinaryStars/Accretion_New/'
list_plot_files = np.genfromtxt(data_dir+'list_plot_files',dtype=str)
list_surf_files = np.genfromtxt(data_dir+'list_surface_files',dtype=str)
...
def get_surf_abunds(surf_data,heavy_yields,asplund_heavy,scaled_asplund_heavy):
    ...
    Full surface abunds is _possible_ for the complete pattern, although only for graphical reasons...
        - it becomes difficult and we have not computed the full elemental pattern here.
    """
...
max_len_mod_array = 30000 # give this a big number to be inclusive
num_params = 4 # Teff, logg, Fe/H, mass
num_elements = 16 # see above for elements
ndims = tuple(len(u) for u in unique) + (max_len_mod_array, num_params + num_elements)
#print(ndims)

print('Populating the MODEL matricies...')
# create massive array to hold all parameter combinations
MODEL = np.zeros(shape=ndims,dtype=np.float32)

# create array to hold valid models; mf < magb
...
for i,(plot_file,surf_file) in enumerate(zip(list_plot_files,list_surf_files)):
    ...
    #print(type(init_mass))
    
    # create indices for model parameters
    ...
    # get the model numbers from plot file
    model_number = acc_data[0].astype(np.int); #print(isinstance(model_number,(float,int,list,tuple,set,range,str,dict,bool)))
    #print(surf_file)
    try:
    ...
    model_metallicity = np.log10(float('0.'+metalZ)/0.0142) * np.ones(n_models) ; #print(model_metallicity) 
    # teff from plot file
    ...
    scaled_asplund_heavy = scale_solar_values(asplund_heavy,input_Z=float('0.'+metalZ))
    ...
    # apply physically motivated cuts to the grid -- this will limit mass ratios to q \leq 1
    if (float(AGB_mass[0]+'.'+AGB_mass[-1])) < float(init_mass):
        continue
    # apply physically motivated cuts to the grid -- this will limit accreted mass to m_acc \leq 0.5
    if float(final_mass) - float(init_mass) > 0.5:
        continue
        
    # append temp and logg info to abundances
    try:
        ...
        # compute size of each grid-cell by multiplying d(theta) 
        #VOLUME[da,db,dc,dd,:n_models] = 
    except:
        print('missing information in ',surf_file)
    

print('writing to file...')
with open('model_grid_big_file_mass_limit_full.dat','wb') as model_handler:
    np.save(model_handler,MODEL)
with open('valid_grid_big_file_mass_limit_full.dat','wb') as valid_handler:
